...
type DeviceQuery struct {
	...
	withThingModel *ThingModelQuery
...
func (_q *DeviceQuery) QueryTenant() *TenantQuery {
...
// QueryThingModel chains the current query on the "thing_model" edge.
func (_q *DeviceQuery) QueryThingModel() *ThingModelQuery {
...
func (_q *DeviceQuery) Clone() *DeviceQuery {
	...
	return &DeviceQuery{
		...
		withThingModel: _q.withThingModel.Clone(),
...
func (_q *DeviceQuery) WithTenant(opts ...func(*TenantQuery)) *DeviceQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
// WithThingModel tells the query-builder to eager-load the nodes that are connected to
// the "thing_model" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *DeviceQuery) WithThingModel(opts ...func(*ThingModelQuery)) *DeviceQuery {
	query := (&ThingModelClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withThingModel = query
	return _q
}
...
func (_q *DeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Device, error) {
	...
	if query := _q.withTenant; query != nil {
		if err := _q.loadTenant(ctx, query, nodes, nil,
			func(n *Device, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withThingModel; query != nil {
		if err := _q.loadThingModel(ctx, query, nodes, nil,
			func(n *Device, e *ThingModel) { n.Edges.ThingModel = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DeviceQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Device, init func(*Device), assign func(*Device, *Tenant)) error {
...
func (_q *DeviceQuery) loadThingModel(ctx context.Context, query *ThingModelQuery, nodes []*Device, init func(*Device), assign func(*Device, *ThingModel)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Device)
	for i := range nodes {
		fk := nodes[i].ThingModelID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(thingmodel.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "thing_model_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *DeviceQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTenant != nil {
			_spec.Node.AddColumnOnce(device.FieldTenantID)
		}
		if _q.withThingModel != nil {
			_spec.Node.AddColumnOnce(device.FieldThingModelID)
		}
	}
