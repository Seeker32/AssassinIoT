...
func (_q *TenantQuery) QueryModelCategories() *ModelCategoryQuery {
...
func (_q *TenantQuery) QueryThingModels() *ThingModelQuery {
...
func (_q *TenantQuery) QueryDevices() *DeviceQuery {
...
// the "model_categories" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TenantQuery) WithModelCategories(opts ...func(*ModelCategoryQuery)) *TenantQuery {
	query := (&ModelCategoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withModelCategories = query
	return _q
}
...
func (_q *TenantQuery) WithThingModels(opts ...func(*ThingModelQuery)) *TenantQuery {
	query := (&ThingModelClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TenantQuery) WithDevices(opts ...func(*DeviceQuery)) *TenantQuery {
	query := (&DeviceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := _q.withModelCategories; query != nil {
		if err := _q.loadModelCategories(ctx, query, nodes,
			func(n *Tenant) { n.Edges.ModelCategories = []*ModelCategory{} },
			func(n *Tenant, e *ModelCategory) { n.Edges.ModelCategories = append(n.Edges.ModelCategories, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withThingModels; query != nil {
		if err := _q.loadThingModels(ctx, query, nodes,
			func(n *Tenant) { n.Edges.ThingModels = []*ThingModel{} },
			func(n *Tenant, e *ThingModel) { n.Edges.ThingModels = append(n.Edges.ThingModels, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDevices; query != nil {
		if err := _q.loadDevices(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Devices = []*Device{} },
			func(n *Tenant, e *Device) { n.Edges.Devices = append(n.Edges.Devices, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TenantQuery) loadModelCategories(ctx context.Context, query *ModelCategoryQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *ModelCategory)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(modelcategory.FieldTenantID)
	}
	query.Where(predicate.ModelCategory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.ModelCategoriesColumn), fks...))
	}))
...
func (_q *TenantQuery) loadThingModels(ctx context.Context, query *ThingModelQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *ThingModel)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(thingmodel.FieldTenantID)
	}
	query.Where(predicate.ThingModel(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.ThingModelsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TenantQuery) loadDevices(ctx context.Context, query *DeviceQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Device)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(device.FieldTenantID)
	}
	query.Where(predicate.Device(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.DevicesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
