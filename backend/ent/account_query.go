...
func (_q *AccountQuery) QueryTenant() *TenantQuery {
...
func (_q *AccountQuery) WithTenant(opts ...func(*TenantQuery)) *AccountQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
//		GroupBy(account.FieldDeletedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withTenant; query != nil {
		if err := _q.loadTenant(ctx, query, nodes,
			func(n *Account) { n.Edges.Tenant = []*Tenant{} },
			func(n *Account, e *Tenant) { n.Edges.Tenant = append(n.Edges.Tenant, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AccountQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Account, init func(*Account), assign func(*Account, *Tenant)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(account.TenantTable)
		s.Join(joinT).On(s.C(tenant.FieldID), joinT.C(account.TenantPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(account.TenantPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(account.TenantPrimaryKey[1]))
